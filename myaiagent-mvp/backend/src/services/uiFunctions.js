...
export const UI_FUNCTIONS = [
  ...
  // Task Management Functions (Planka)
  {
    name: 'createTask',
    description: 'Create a task/card in the project management system. Use this when the user says things like: "remind me to...", "create a task for...", "add a todo for...", "I need to remember to...", or explicitly asks to create a task.',
    parameters: {
      type: 'object',
      properties: {
        name: {
  ...
  {
    name: 'listTasks',
    description: 'List all tasks/cards for the current user. Use this when the user asks: "what tasks do I have?", "show my todos", "what do I need to do?", "list my tasks".',
    parameters: {
      type: 'object',
      properties: {},
      required: [],
  ...
  {
    name: 'updateTask',
    description: 'Update a task/card (mark as complete, move to different status, update details). Use when user wants to complete, update, or change task status.',
    parameters: {
      type: 'object',
      properties: {
        cardId: {
        ...
        listName: {
          ...
          description: 'Move task to different list/status (optional)',
        },
      },
      required: ['cardId'],
    },
...
export async function executeUIFunction(functionName, args, context) {
  ...
  if (functionName === 'createTask') {
    const { plankaService } = await import('./planka.js');
    ...
    try {
      ...
      const card = await plankaService.createCard(plankaUserId, context.user.id, context.user.email, {
        name: args.name,
      ...
      if (conversationId) {
        await plankaService.linkCardToConversation(card.id, conversationId, context.user.id, plankaUserId);
      }

      return {
        success: true,
        message: `Task "${args.name}" created successfully`,
        data: { card },
      };
    } catch (error) {
      return {
  ...
  if (functionName === 'listTasks') {
    ...
    try {
      ...
      const cards = await plankaService.getCards(plankaUserId, context.user.email);

      return {
        success: true,
        message: `Found ${cards.length} task(s)`,
        data: { tasks: cards },
      };
    } catch (error) {
      return {
  ...
  if (functionName === 'updateTask') {
    ...
    try {
      ...
      const card = await plankaService.updateCard(args.cardId, plankaUserId, {
        name: args.name,
